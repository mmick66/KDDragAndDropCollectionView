...
class ViewController: UIViewController, KDDragAndDropCollectionViewDataSource {
  ...
  override func viewDidLoad() {
    ...
    self.dragAndDropManager =
      KDDragAndDropManager(canvas: self.view,
  ...
  // The cell that is returned must be retrieved from
  // a call to -dequeueReusableCellWithReuseIdentifier:forIndexPath:
  func collectionView(collectionView: UICollectionView,
    cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {

      guard let cell = collectionView.dequeueReusableCellWithReuseIdentifier("Cell",
        forIndexPath: indexPath) as? ColorCell else {
  ...
  func collectionView(collectionView: UICollectionView, insertDataItem dataItem: AnyObject,
    atIndexPath indexPath: NSIndexPath) -> Void {
  ...
  func collectionView(collectionView: UICollectionView, deleteDataItemAtIndexPath
    indexPath: NSIndexPath) -> Void {
      data[collectionView.tag].removeAtIndex(indexPath.item)
  }

  func collectionView(collectionView: UICollectionView, moveDataItemFromIndexPath from: NSIndexPath,
    toIndexPath: NSIndexPath) -> Void {
      ...
      if let candidate: DataItem = dataItem as? DataItem {
        ...
        for item: DataItem in data[collectionView.tag] {
          if candidate  == item {
            ...
            let position = data[collectionView.tag].indexOf(item)!
            // ! if we are inside the condition we are guaranteed a position
